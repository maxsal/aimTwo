#' Modified version of the function wlasso from the wlasso package that accommodates manually specified lambda.grid
#'
#'@description The wlasso function from the wlasso package currently throws an error if lambda.grid is manually specified. This modification circumvents this error. FROM WLASSO: This function allows as to fit LASSO prediction (linear or logistic) models to complex survey data, considering sampling weights in the estimation process and selects the lambda that minimizes the error based on different replicating weights methods.
#'
...
#' @export
weighted_lasso <- function(data = NULL, col.y = NULL, col.x = NULL,
  ...
  newdata <- wlasso::replicate.weights(data = data, method = method,
  ...
  for(col.w in rwtraincols){
    ...
    # Sample yhat
    yhat <- stats::predict(model, newx=as.matrix(newdata[,col.x]), type = "response")
  ...
  error <- wlasso:::error.f(data = newdata, l.yhat = l.yhat,
  ...
  result <- list(lambda.grid = lambda.grid,
                 ...
                 all.error = error,
                 model.min = model)
  if (!is.null(model.orig)) {
    result$model.grid <- model.orig
  }
